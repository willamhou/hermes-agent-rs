...
- MCP bridge tools cover prompts and resources for all transports; resource subscribe/unsubscribe is currently exposed for `stdio` MCP servers, and both stdio plus supported HTTP SSE notification streams refresh the registry after `notifications/*/list_changed`.
