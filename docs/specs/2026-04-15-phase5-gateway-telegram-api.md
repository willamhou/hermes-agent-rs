# Phase 5: Gateway — Telegram + API Server — Design Spec
...
impl GatewayRunner {
    pub async fn run(&self) -> Result<()> {
        ...
        if let Some(tg_config) = &self.config.telegram {
            let adapter = TelegramAdapter::new(tg_config.clone())?;
        ...
        if let Some(api_config) = &self.config.api_server {
            let adapter = ApiServerAdapter::new(api_config.clone());
            let tx = event_tx.clone();
            adapter_handles.push(tokio::spawn(async move {
                adapter.start(tx).await
            }));
...
async fn session_task(
    ...
    // Build Agent (provider, registry, memory, etc.)
    let mut agent = build_agent(&session_id, &app_config);
    let mut history = Vec::new();

    while let Some(routed) = msg_rx.recv().await {
        let (delta_tx, _delta_rx) = mpsc::channel(64); // gateway discards deltas
        ...
        if let Some(adapter) = adapters.get(&routed.event.platform) {
            let _ = adapter.send_response(&routed.event, &response).await;
        }

        // For API server: send response via oneshot
...
async fn start(&self, event_tx: mpsc::Sender<PlatformEvent>) -> Result<()> {
    ...
    loop {
        let updates = self.get_updates(offset, 30).await?; // 30s long-poll timeout
...
`split_message()`: split at newline boundaries when possible, not mid-word.
...
3. Send to `event_tx`
4. Wait for response via `oneshot` channel (sync response)
5. Return JSON
